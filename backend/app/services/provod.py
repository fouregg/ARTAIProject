...
def _error_from_response(response: httpx.Response) -> ProvodError:
    ...
    if status in (401, 403) and "TOP_UP" not in normalized:
        ...
        # Так шлюз оформляет отказ рисующей модели. Проверено запросами к шлюзу:
        # срабатывает на защищённых персонажах (Микки Маус, Человек-паук), тогда как
        # бренды и реальные люди проходят. Поэтому и говорим гостю про авторские права.
        message = (
            "Сервис не может сгенерировать изображение из-за авторских прав. "
            "Попробуйте описать сцену своими словами, без известных персонажей."
        )
        code = "IMAGE_REJECTED"
